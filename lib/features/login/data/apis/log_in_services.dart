...
class LogInServices {
  Future<LoginResults> signInWithEmailAndPassword(LoginCredentials credentials) async {
    try {
  ...
  Future<LoginResults> signInWithGoogle() async {
    try {
      ...
      if (googleUser == null) {
        return LoginResults(success: false, errorMessage: "Google Sign-In was cancelled by the user.");
      }
      ...
      if (userCredential.user?.email == null || userCredential.user?.displayName == null) {
        return LoginResults(success: false, errorMessage: "Missing user information.");
      }
      ...
      await SharedPrefHelper.setData(SharedPrefKeys.userUid, userCredential.user?.uid);
      return LoginResults(success: true, user: userCredential.user);
    } catch (e) {
      return LoginResults(success: false, errorMessage: e.toString());
    }
